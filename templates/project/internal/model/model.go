...
import (
	...
	"goprojects/internal/common/enum/employee_enum"
...
FindSysMenu:
	for i := 0; i < len(apiUrls); i++ {
		sysMenu := sys.SysMenu{}
		has, err := DB.Where("api_urls like ?", "%"+apiUrls[i].ApiUrl+"%").FindOne(&sysMenu)
		if err != nil {
			return err
		}
	...
	for i := 0; i < len(sysMenus); i++ {
		...
		if _, err = DB.InsertOneTx(session, roleMenu); err != nil {
			session.Rollback()
			return err
		}
	}
