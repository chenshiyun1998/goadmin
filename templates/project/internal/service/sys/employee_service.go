...
func (self *EmployeeService) Get(id int64) (*sys.Employee, error) {
	...
	has, err := DB.FindById(id, &employee)
	if err != nil {
		return nil, err
	}
	if !has {
...
func (EmployeeService) GetByAccount(account, password string) (employee *sys.Employee, err error) {
	var ok bool
	employee = &sys.Employee{}
	ok, err = DB.Where("`account`=? and `password`=?", account, password).Omit("password").FindOne(employee)
	if err != nil {
		return nil, err
	}
	if !ok {
...
func (self *EmployeeService) FindById(id int64) (*sys.Employee, error) {
	...
	ok, err := DB.FindById(id, employee)
	if !ok {
...
func (EmployeeService) SaveTx(session *xorm.Session, employee *sys.Employee) (err error) {
	...
	if employee.Id == 0 {
		employee.Password = gosecurity.MD5Password(employee.Password)
		_, err = DB.InsertOneTx(session, employee)
	} else {
...
func (self *EmployeeService) ResetPassword(account string) (err error) {
	...
	employee := new(sys.Employee)
	ok, err := DB.Where("`account`=? ", account).FindOne(employee)
	if err != nil {
		return err
	}
	if !ok {
	...
	employee.Password = gosecurity.MD5Password("111111")
...
func (self *EmployeeService) UpdatePassword(req *sys.PasswordReq) (err error) {
	...
	employee := new(sys.Employee)
	ok, err := DB.Where("`account`=? and `password`=?", req.Account, gosecurity.MD5Password(req.OldPassword)).FindOne(employee)
	if err != nil {
		return err
	}
	if !ok {
	...
	employee.Password = gosecurity.MD5Password(req.Password)
	_, err = DB.UpdateById(employee.Id, employee)
